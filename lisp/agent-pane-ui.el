...
(defun agent-pane--ui-update-status-line ()
  ...
  (let ((s (agent-pane--ui-status-line)))
    ...
    (when agent-pane-show-header-line-status
      ...
      (setq-local header-line-format s))
...
INPUT is the current input contents to restore (or nil)."
  ...
  (agent-pane--insert-read-only "Type your message below and press C-c C-c\n\n")
...
(defun agent-pane--render-header ()
  ...
  "Insert a status/debug header for the current ACP connection.

This header is intentionally verbose and exposes much of what `acp.el'

tracks: negotiated capabilities, client/process details, and the log/

traffic buffers created by `acp.el'."
  ...
  (when agent-pane-show-acp-header
    ...
    (let* ((client (map-elt agent-pane--state :client))
           ...
           (subscribed (map-elt agent-pane--state :subscribed))
      ...
      (agent-pane--insert-read-only
       ...
       (propertize (format "ACP: %s  (acp.el %s, logging=%s)\n"

                           status

                           (if (boundp 'acp-package-version) acp-package-version "?")

                           (if acp-logging-enabled "on" "off"))

                   'face 'agent-pane-role-acp)
      ...
      (agent-pane--insert-read-only
       ...
       (format "  auth=%s  mode=%s  permissions=%s\n"
       ...
       'agent-pane-raw (list :auth auth :mode mode-result))
      ...
      (agent-pane--insert-read-only
       ...
       "Keys: C-c C-c send | C-c C-t traffic | C-c C-l logs | C-c C-e stderr | C-c C-s state | C-c C-r raw\n"
